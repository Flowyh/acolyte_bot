...
@bot.command(name='votekick', usage='{@user} {time in seconds}',
             brief='Vote to kick desired user. More info !help votekick',
             help='Vote to kick desired user. Passes only if 2/3 vc members voted for yes. 30 min cooldown per user.')
@commands.cooldown(1, 1800, type=commands.BucketType.user)
async def votekick(ctx: discord.ext.commands.Context, user: str, time=30):
    author = ctx.message.author
    victim_id = int(user.replace('<', '').replace('@', '').replace('>', '').replace('!', ''))
    victim = await bot.fetch_user(user_id=victim_id)
